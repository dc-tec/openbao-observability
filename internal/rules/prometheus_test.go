...
func TestGeneratePrometheusRules(t *testing.T) {
	...
	required := []string{
		...
		"expr: sum by (namespace) (openbao_core_active)",
		"expr: sum by (namespace) (openbao_core_unsealed{cluster!=\"\"})",
		"record: openbao:core_handle_request:rate5m",
		"expr: sum by (namespace) (rate(openbao_core_handle_request_count[5m]))",
		"record: openbao:core_handle_login_request:avg5m",
		"expr: sum by (namespace) (rate(openbao_core_handle_login_request_sum[5m])) / " +
			"clamp_min(sum by (namespace) (rate(openbao_core_handle_login_request_count[5m])), 0.001)",
		"record: openbao:core_check_token:avg5m",
		"expr: sum by (namespace) (rate(openbao_core_check_token_sum[5m])) / " +
			"clamp_min(sum by (namespace) (rate(openbao_core_check_token_count[5m])), 0.001)",
		"record: openbao:core_in_flight_requests:max",
		"expr: max by (namespace) (openbao_core_in_flight_requests)",
		...
		"expr: clamp_min(max by (namespace, instance, peer_id) (openbao_raft_storage_stats_commit_index - " +
			"openbao_raft_storage_stats_applied_index), 0)",
		"record: openbao:audit_log_request:avg5m",
		"expr: sum by (namespace) (rate(openbao_audit_log_request_sum[5m])) / " +
			"clamp_min(sum by (namespace) (rate(openbao_audit_log_request_count[5m])), 0.001)",
		"record: openbao:expire_num_irrevocable_leases:max",
		"expr: max by (namespace) (openbao_expire_num_irrevocable_leases)",
		"record: openbao:token_count:max30m",
		"expr: max by (namespace) (max_over_time(openbao_token_count[30m]))",
		...
		"expr: sum by (namespace) (rate(openbao_token_lookup_sum[5m])) / " +
			"clamp_min(sum by (namespace) (rate(openbao_token_lookup_count[5m])), 0.001)",
		"record: openbao:runtime_heap_objects:max",
		"expr: max by (namespace) (openbao_runtime_heap_objects)",
		"record: openbao:barrier_get:avg5m",
		"expr: sum by (namespace) (rate(openbao_barrier_get_sum[5m])) / " +
			"clamp_min(sum by (namespace) (rate(openbao_barrier_get_count[5m])), 0.001)",
		"record: openbao:cache_hit_ratio:ratio5m",
		"expr: sum by (namespace) (rate(openbao_cache_hit[5m])) / " +
			"clamp_min(sum by (namespace) (rate(openbao_cache_hit[5m])) + " +
			"sum by (namespace) (rate(openbao_cache_miss[5m])), 0.001)",
		...
		"expr: sum by (namespace) (max by (namespace, mount_point) (max_over_time(openbao_secret_kv_count[30m])))",
		"record: openbao:secret_kv_count_by_namespace:max30m",
		"expr: sum by (namespace) (max by (namespace, mount_point) (max_over_time(openbao_secret_kv_count[30m])))",
		"record: openbao:route_create_kv_v1:rate5m",
		"expr: sum by (namespace) (rate(openbao_route_create_kv_v1__count[5m]))",
		"record: openbao:route_read_kv_v1:avg5m",
		"expr: sum by (namespace) (rate(openbao_route_read_kv_v1__sum[5m])) / " +
			"clamp_min(sum by (namespace) (rate(openbao_route_read_kv_v1__count[5m])), 0.001)",
		"record: openbao:route_delete_kv_v1:rate5m",
		"expr: sum by (namespace) (rate(openbao_route_delete_kv_v1__count[5m]))",
		"record: openbao:route_create_kv_v2:rate5m",
		"expr: sum by (namespace) (rate(openbao_route_create_secret__count[5m]))",
		"record: openbao:route_read_kv_v2:avg5m",
		"expr: sum by (namespace) (rate(openbao_route_read_secret__sum[5m])) / " +
			"clamp_min(sum by (namespace) (rate(openbao_route_read_secret__count[5m])), 0.001)",
		"record: openbao:route_delete_kv_v2:rate5m",
		"expr: sum by (namespace) (rate(openbao_route_delete_secret__count[5m]))",
		"record: openbao:pki_issue:avg5m",
		"expr: sum by (namespace) (rate(openbao_pki_issue_sum[5m])) / " +
			"clamp_min(sum by (namespace) (rate(openbao_pki_issue_count[5m])), 0.001)",
		"record: openbao:pki_issue_failure:increase15m",
		"expr: sum by (namespace) (increase(openbao_pki_issue_failure[15m]))",
		"record: openbao:database_new_user:rate5m",
		"expr: sum by (namespace) (rate(openbao_database_NewUser_count[5m]))",
		"record: openbao:database_new_user_error:increase15m",
		"expr: sum by (namespace) (increase(openbao_database_NewUser_error[15m]))",
