...
func fixtureProfiles(root string, contract *contracts.MetricContract) ([]fixtureProfile, error) {
	...
	for _, prefix := range contract.MetricPrefixes.Supported {
		profile := fixtureProfile{
			Name:   prefix + "-prefix",
			Prefix: prefix,
			Path:   filepath.Join(root, "metrics", fmt.Sprintf("openbao-%s-%s-prefix.prom", contract.OpenBAOVersion, prefix)),
		}
	...
	paths, err := filepath.Glob(filepath.Join(root, "metrics", fmt.Sprintf("openbao-%s-*.prom", contract.OpenBAOVersion)))
	if err != nil {
...
func renderMatrix(opts MatrixOptions, contract *contracts.MetricContract, profiles []fixtureProfile, familiesByProfile map[string]promtext.Families) []byte {
	var buf bytes.Buffer

	fmt.Fprintln(&buf, "# OpenBao metric compatibility matrix")
	fmt.Fprintln(&buf)
	fmt.Fprintln(&buf, "This generated reference lists each metric contract entry against captured OpenBao metric fixtures. Regenerate it with `make generate` after changing metric contracts or fixture captures.")
	fmt.Fprintln(&buf)
	fmt.Fprintln(&buf, "> [!WARNING]")
	fmt.Fprintln(&buf, "> Do not edit this file by hand. Edit the metric contract or captured fixtures and regenerate the matrix.")
	fmt.Fprintln(&buf)
	...
	fmt.Fprintln(&buf)
	fmt.Fprintln(&buf, "| OpenBao version | Profile | Prefix | Metric ID | Docs metric | Source metric | Status | Type | Labels | Required | Overview | Notes |")
	fmt.Fprintln(&buf, "| --------------- | ------- | ------ | --------- | ----------- | ------------- | ------ | ---- | ------ | -------- | -------- | ----- |")
