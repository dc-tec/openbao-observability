...
func run(out io.Writer, errOut io.Writer, govulncheckPath, ignorePath string, showIgnored bool, patterns []string) int {
	...
	if len(unignored) == 0 {
		return handleIgnoredFindings(out, errOut, govulncheckPath, showIgnored, patterns, found)
	}
	return handleUnignoredFindings(out, errOut, govulncheckPath, ignorePath, patterns, unignored)
}
...
func runGovulncheck(path string, args ...string) (combined []byte, rc int, err error) {
	...
	// #nosec G204 -- path is the repository-managed govulncheck binary or an explicit maintainer override.
...
func loadIDs(path string) (map[string]bool, error) {
	b, err := os.ReadFile(path) // #nosec G304 -- path is the repository-local ignore file or CLI override.
	if err != nil {
